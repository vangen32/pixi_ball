...
export class Engine {
  ...
  private onUpdateHandler: EngineUpdateCallback = (ballPosition: Position, ballRadius: number) => {};
  ...
  private update() {
    this._ballPosition.update(this._ballPosition.x + this.velocity.x, this._ballPosition.y + this.velocity.y);
    this._speed *= 0.99;
    if (this._speed < 0.1) {
      this._speed = 0;
      clearInterval(this._intervalId);
    ...
    if (
      ...
      this._ballPosition.y + this.ballRadius >= this.fieldSize.height
    ) {
      this.velocity.y *= -1;
